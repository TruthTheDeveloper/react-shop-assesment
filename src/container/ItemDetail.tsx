...
const ItemDetail = () => {
  ...
  const addToCartHandler = () => {
    ...
    singleProduct && dispatch(addTocart({
      id:singleProduct?.id,
  ...
  return (
    <div className="flex mx-28 mt-16">
      ...
      <div className=" basis-1/2 pl-56 ">
        ...
        <div className="mb-4">
          ...
          <div className="flex">
            {singleProduct?.size.map((el, index) => (
              el === selectedSize ? <div key={index} onClick={() => setSelectedSize(el)} className="px-3 py-1 border bg-black border-black mr-2 text-white cursor-pointer">{el}</div>:
              <div onClick={() => setSelectedSize(el)} className="px-3 py-1 border border-black mr-2 cursor-pointer">{el}</div>
            ))}
          </div>
        ...
        <button className="py-3 px-8 bg-green-400 text-white mb-4" onClick={addToCartHandler}>
          ADD TO CART
        </button>
